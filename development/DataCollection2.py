...
class BatchRandomInputGenerator(object):
          ...
          def generateBatch(self):
                    ...
                    for i in range(len(self.ranges)):
                              randoms.append(np.full((1, self.batchsize),
                                                     np.random.uniform(self.ranges[i][0], self.ranges[i][1], size=1)[
                                                               0]))
...
class DataCollection2(object):
          ...
          def __iadd__(self, other):
                    ...
                    if self.dataDir != other.dataDir:
                              raise ValueError(
                                        'The two DataCollections have different data directories, still to be implemented!')
                    ...
                    if type(self.dataclass) != type(other.dataclass):
                              raise ValueError(
                                        ...
                                        ' different data class type! (%s, and %s)' % (
          ...
          def validate(self, remove=True, skip_first=0):
                    ...
                    for i in range(len(self.samples)):
                              ...
                              print('reading ' + fullpath, str(self.sampleentries[i]), str(i), '/',
          ...
          def split(self, ratio):
                    ...
                    if nsamplefiles < 2:
                              ...
                              print(
                                        'DataCollection.split: warning: only one file, split will just return a copy of this')
                              return out
          ...
          def generator(self):
                    ...
                    class tdreader(object):
                              ...
                              def start(self):
                                        if self.max < 1:
                                                  raise ValueError(
                                                            'I got an invalid number of files to open (%d)' % self.max)
                              ...
                              def __readNext(self):
                                        ...
                                        def startRead(counter, filename, shuffleseed):
                                                  ...
                                                  while excounter < 10:
                                                            try:
                                                                      self.tdlist[counter].readIn_async(filename,
                                                                                                        ramdiskpath='/dev/shm/',
                                                                                                        randomseed=shuffleseed)
                                                                      break
                                                            except Exception as d:
                                                                      print(self.filelist[
                                                                                      counter] + ' read error, retry...')
                                        ...
                                        t = threading.Thread(target=startRead,
                                                             args=(self.nextcounter, readfilename, self.shuffleseed))
                                        t.start()
                                        self.shuffleseed += 1
