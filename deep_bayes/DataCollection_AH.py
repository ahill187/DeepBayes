...
import copy
from data_helper_functions import Bins, BinClass
...
class DataCollection(object):
          ...
          def setBinWeighting(self, custom, weights):
                    self.custom = custom
          ...
          def getNBatchesPerEpoch(self, custom=True, nbatches=1):
          ...
          def generator(self):
                    ...
                    while 1:
                              ...
                              if self.useweights:
                                        ...
                                        y = []
                                        for i in range(0, len(yout[0])):
                                                  y.append(yout[0][i][0])
                                        if self.firsttime == 1:
                                                  self.minbin = min(y)
                                                  self.maxbin = max(y)
                                                  self.ybins = Bins(self.getBins(), self.minbin, self.maxbin, custom=self.custom, weights=self.bin_weights)
                                                  self.firsttime = 0
                                        #print(self.custom)
                                        #print(self.bin_weights)
                                        #print(self.ybins)
                                        y = BinClass(y, self.bins, self.ybins)
                                        yout_hot = []
                                        yout_hot.append(y)
                                        # print("yout_hot=")
                                        # print(yout[0][0])
...
# def Basic2():
#           i=0
#           for xout, yout in train.train_data.generator():
#                     y = []
#                     for i in range(0, len(yout[0])):
#                               y.append(yout[0][i][0])
#                     ybins = Bins(bins, -2, 2)
...
#                               break
#           yield (xout, yout)
# model.fit_generator(Basic(), 9, epochs=5)
