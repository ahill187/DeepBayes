...
class training_base(object):
          def __init__(self, args, splittrainandtest=0.85, useweights=False, testrun=False, resumeSilently=False,
		        renewtokens=True, collection_class=DataCollection, pythonconsole=False, treefiles=""):
                    import sys
          ...
          def setModel(self,model, custom=False, bin_weights=[], **modelargs):
                    ...
                    self.train_data.setBinWeighting(custom, bin_weights)
          ...
          def compileModel(self, learningrate, clipnorm=None, **compileargs):
                    ...
                    if clipnorm:
                              self.optimizer = Nadam(lr=self.startlearningrate,clipnorm=clipnorm)
                    else:
                              self.optimizer = Nadam(lr=self.startlearningrate)
          ...
          def trainModel(self, nepochs, batchsize, stop_patience=-1,  lr_factor=0.5, lr_patience=-1,  lr_epsilon=0.003,
                   lr_cooldown=6, lr_minimum=0.000001, maxqsize=5, checkperiod=10, additional_plots=None, custom=False, bin_weights=[],
                         nbatches=1,**trainargs):
                    ...
                    self.train_data.setNBatches(nbatches)
                    self.val_data.setNBatches(1)
                    ...
                    #                 checkperiod=checkperiod,
                    #                 checkperiodoffset=self.trainedepoches,
                    #                 additional_plots=None, plotLossEachEpoch=False)

                    print('starting training')
                    self.val_data.setBatchSize(1)
