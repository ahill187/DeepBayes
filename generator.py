...
# Testing Data -----------------------------------------------------------------------------------
sd = round(float(np.random.uniform(0, 1)), 8)
test = CreateExampleData(title="ExampleTestingData.pdf", bins=bins, sd=sd, sd_smear=0.3)
plt.close('all')
# Keras Model -------------------------------------------------------------------------------------
...
def BayesIteration(multi, train, bins):
    ...
    while True:
        ...
        sd = round(float(np.random.uniform(0, 1)), 8)
        test = CreateExampleData(title="ExampleTestingData_"+str(k)+".pdf", bins=bins, sd=sd, sd_smear=0.3)
        plt.close('all')
...
model = BayesIteration(multi, train, bins)
